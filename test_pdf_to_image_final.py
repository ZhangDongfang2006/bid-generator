...
with tempfile.TemporaryDirectory() as temp_dir:
    ...
    if result.returncode == 0 and output_path.exists():
        ...
        import shutil
        shutil.copy(output_path, final_img_path)
        print(f"✓ 图片已保存到：{final_img_path}")
        print(f"  文件大小：{final_img_path.stat().st_size} 字节")
        print()
        ...
        print(f"  文件大小：{final_doc_path.stat().st_size} 字节")
        print()
        ...
        print("=" * 80)
        print()
        print("1. 查看 output 目录中的文件：")
        print(f"   {output_dir}")
        print()
        print("2. 打开 test_doc.docx 文件")
        print("   查看是否包含证书图片")
        print()
        print("3. 如果有图片，说明 PDF 转图片功能正常")
        print()
        print("=" * 80)
        print()
        sys.exit(0)
